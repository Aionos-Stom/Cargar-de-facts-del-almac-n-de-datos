...
**IMPORTANTE: Configura tu base de datos en los siguientes archivos:**

1. **SalesAnalysis.Worker/appsettings.json**
   ```json
   "ConnectionStrings": {
     "DefaultConnection": "Server=TU_SERVIDOR;Database=BusinessIntelligenceDW;Trusted_Connection=true;TrustServerCertificate=true;"
   }
   ```

2. **SalesAnalysis.Web/appsettings.json**
   ```json
   "ConnectionStrings": {
     "DefaultConnection": "Server=TU_SERVIDOR;Database=BusinessIntelligenceDW;Trusted_Connection=true;TrustServerCertificate=true;"
   }
...
3. **SalesAnalysis.Persistence/Services/ComprehensiveEtlService.cs** (líneas 184 y 202)
   - Reemplaza `localhost` con el nombre de tu servidor SQL Server
   - Asegúrate de que la base de datos `BusinessIntelligenceDW` exista

**Nota:** Si usas autenticación SQL Server en lugar de Windows Authentication, cambia `Trusted_Connection=true` por `User Id=tu_usuario;Password=tu_password;`
...
Ejecuta las migraciones de Entity Framework Core para crear las tablas:
